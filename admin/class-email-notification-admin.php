...
class Email_Notification_Admin {
	...
	function default_before_after_months_cb(){
		echo '<input type="number" required name="default_before_after_months" id="default_before_after_months" placeholder="--" value="'.get_option('default_before_after_months', '').'">';
	}
	...
	function extra_before_after_months1_cb(){
		...
		echo '<input type="number" required name="extra_before_after_months1[cond]" value="'.$cond.'" placeholder="--">&nbsp;';
		echo '<input type="email" required name="extra_before_after_months1[email]" value="'.$email.'" placeholder="email">';
	...
	function extra_before_after_months2_cb(){
		...
		echo '<input type="number" required name="extra_before_after_months2[cond]" value="'.$cond.'" placeholder="--">&nbsp;';
		echo '<input type="email" required name="extra_before_after_months2[email]" value="'.$email.'" placeholder="email">';
	...
	function extra_before_after_months3_cb(){
		...
		echo '<input type="number" required name="extra_before_after_months3[cond]" value="'.$cond.'" placeholder="--">&nbsp;';
		echo '<input type="email" required name="extra_before_after_months3[email]" value="'.$email.'" placeholder="email">';
	...
	function emailSendingProcess($data){
		global $wpdb;
		...
		$unsubscribeLink .= "?date=".base64_encode($currentRecord);

		$subject = (($data['subject']) ? $data['subject'] : 'Your registered date is expired!');
		$email_body = $data['email_body'];
		$logo_url = ((get_option('en_email_logo_url')) ? get_option('en_email_logo_url') : '');
		$email_footer = $data['email_footer'];

		$transDate = english_to_german_month(date("F, Y", strtotime($data['date'])));

		$username = explode("@", $data['email'])[0];

		$subject = str_replace("{username}", ucfirst($username), $subject );
		$subject = str_replace("{text}", '<strong>'.$text.'</strong>', $subject );
		$subject = str_replace("{date}", '<strong>'.$transDate.'</strong>', $subject );
		...
		$body = $this->email_template($emailData);
		$headers = array('Content-Type: text/html; charset=UTF-8');
		if(wp_mail( $data['email'], $subject, $body, $headers )){
			$wpdb->update($wpdb->prefix.'email_notifications', array(
				'notified' => $data['currentDate']
			), array('ID' => $currentRecord), array('%s'), array('%d'));

			$defaultCondition = get_option( 'default_before_after_months' );
			$extra1 = get_option('extra_before_after_months1');
			$extra2= get_option('extra_before_after_months2');
			$extra3 = get_option('extra_before_after_months3');
			
			$renew = true;
			if($defaultCondition !== false){
				if(!get_option("default_notification_pass_$currentRecord")){
					$renew = false;
				}
			}
			if($extra1 !== false){
				if(!get_option("extra1_notification_pass_$currentRecord")){
					$renew = false;
				}
			}
			if($extra2 !== false){
				if(!get_option("extra2_notification_pass_$currentRecord")){
					$renew = false;
				}
			}
			if($extra3 !== false){
				if(!get_option("extra3_notification_pass_$currentRecord")){
					$renew = false;
				}
			}
			
			if($renew){
				$mdate = date("Y-m-d", strtotime($data['currentDate']. "+2 years"));
				$wpdb->update($wpdb->prefix.'email_notifications', ['date' => $mdate], ['ID' => $currentRecord], ['%s'], ['%d'] );
			}
		}
	}
	...
	function send_email_notifications(){
		...
		$currentDate = date("Y-m", strtotime($currentDate))."-01";
		$upcommingDates = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}email_notifications WHERE '$currentDate' > DATE(notified)");
		...
		if($upcommingDates){
			foreach($upcommingDates as $upcomming){
				...
				$currentRecord = $upcomming->ID;

				// Default
				$defaultCalculatedDate = calculateDate($defaultCondition, $upDate);
				$defaultNotification = strtotime($currentDate) === strtotime($defaultCalculatedDate);
				...
				if($defaultNotification){
					...
					$data['text'] = $upcomming->text;
					$data['subject'] = get_option('en_email_subject');
					$data['email_body'] = $email_body;
					$data['email_footer'] = $email_footer;
					$data['date'] = $upcomming->date;
					$data['email'] = $upcomming->email;
					$data['currentDate'] = $currentDate;
					// Sent email
					$this->emailSendingProcess($data);
					update_option("default_notification_pass_$currentRecord", "true");
				}

				// extra1
				$cond1 = ((is_array($extra1) && array_key_exists('cond', $extra1)) ? $extra1['cond']: '');
				$extra1CalculatedDate = calculateDate($cond1, $upDate);
				$extra1Notification = strtotime($currentDate) === strtotime($extra1CalculatedDate);
				if($extra1Notification){
					$email = ((is_array($extra1) && array_key_exists('email', $extra1)) ? $extra1['email']: null);
					$email_body = ((is_array($extra1) && array_key_exists('text', $extra1)) ? $extra1['text']: null);
					$subject = get_option( 'en_email_subject1' );
					$footer = get_option( 'en_email_footer1' );
					...
					$data['email_body'] = $email_body;
					$data['email_footer'] = $footer;
					$data['date'] = $upcomming->date;
					$data['email'] = $email;
					$data['currentDate'] = $currentDate;
					// Sent email
					$this->emailSendingProcess($data);
					update_option("extra1_notification_pass_$currentRecord", "true");
				}

				// extra2
				$cond2 = ((is_array($extra2) && array_key_exists('cond', $extra2)) ? $extra2['cond']: '');
				$extra2CalculatedDate = calculateDate($cond2, $upDate);
				$extra2Notification = strtotime($currentDate) === strtotime($extra2CalculatedDate);
				if($extra2Notification){
					$email = ((is_array($extra2) && array_key_exists('email', $extra2)) ? $extra2['email']: null);
					$email_body = ((is_array($extra2) && array_key_exists('text', $extra2)) ? $extra2['text']: null);
					$subject = get_option( 'en_email_subject2' );
					$footer = get_option( 'en_email_footer2' );
					...
					// Sent email
					$this->emailSendingProcess($data);
					update_option("extra2_notification_pass_$currentRecord", "true");
				}

				// extra3
				$cond3 = ((is_array($extra3) && array_key_exists('cond', $extra3)) ? $extra3['cond']: '');
				$extra3CalculatedDate = calculateDate($cond3, $upDate);
				$extra3Notification = strtotime($currentDate) === strtotime($extra3CalculatedDate);
				if($extra3Notification){
					$email = ((is_array($extra3) && array_key_exists('email', $extra3)) ? $extra3['email']: null);
					$email_body = ((is_array($extra3) && array_key_exists('text', $extra3)) ? $extra3['text']: null);
					$subject = get_option( 'en_email_subject3' );
					$footer = get_option( 'en_email_footer3' );
					...
					// Sent email
					$this->emailSendingProcess($data);
					update_option("extra3_notification_pass_$currentRecord", "true");
				}
			}
		}
	}
